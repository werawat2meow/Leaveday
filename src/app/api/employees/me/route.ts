...
function startOfNextYearUTC(y: number) {
  ...
  try {
    ...
    if (!employee && (session.user as any)?.id) {
      ...
      if (!isNaN(userIdFromSession)) {
        ...
        console.log(
          `Warning: session.user.id is not a valid number: ${
    ...
    if (
      entitled.vacation + entitled.business + entitled.sick === 0 &&
      employee.prefix
    ) {
      const template = await prisma.leaveRightsTemplate.findFirst({
        where: { prefix: employee.prefix },
      });
      if (template) {
    ...
    for (const l of approved) {
      ...
      let days =
        typeof l.requestedDays === "number"
          ? l.requestedDays
          : dayDiffInclusive(new Date(l.startDate), new Date(l.endDate));
      switch (l.kind) {
        case "ANNUAL":
          used.vacation += days;
          break;
        ...
        case "BIRTHDAY":
          ...
          break;
        case "ORDAIN":
          used.ordainDays += days;
          break;
        case "MATERNITY":
          used.maternity += days;
          break;
        case "ANNUAL_HOLIDAY":
          used.annualHolidays += days;
          break; // ลาโดยใช้วันหยุดประจำปี
